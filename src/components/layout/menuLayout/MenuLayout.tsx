...
export default function MenuLayout({ theme, updateTheme }: Props) {
  ...
  return (
    <div className="menu-layout">
      ...
      {/* Styling modale */}
      <Modal title={t('settings')} open={stylingOpen} onClose={() => setStylingOpen(false)} className="styling-modal">
        <div>
        ...
        <div>
          <Text>{t('colors')}</Text>
          <Button onClick={() => updateTheme(lightTheme)}>{t('light theme')}</Button>
          <Button onClick={() => updateTheme(lightContrastTheme)}>{t('light contrast theme')}</Button>
          <Button onClick={() => updateTheme(darkTheme)}>{t('dark theme')}</Button>
          <Button onClick={() => updateTheme(darkContrastTheme)}>{t('dark contrast theme')}</Button>
          <Accordion label={t('more')}>
            <div>
              <Label width="100px">{t('Primary')}</Label>
              <ColorPicker value={theme.primary} onChange={(value) => updateTheme({ primary: value })} />
            </div>
            <div>
              <Label width="100px">{t('Primary text')}</Label>
              <ColorPicker value={theme.textPrimary} onChange={(value) => updateTheme({ textPrimary: value })} />
            </div>
            <div>
              <Label width="100px">{t('Surface')}</Label>
              <ColorPicker value={theme.surface} onChange={(value) => updateTheme({ surface: value })} />
            </div>
            <div>
              <Label width="100px">{t('Surface text')}</Label>
              <ColorPicker value={theme.textSurface} onChange={(value) => updateTheme({ textSurface: value })} />
            </div>
          </Accordion>
        </div>
        ...
        <div>
          ...
          <Button onClick={() => updateTheme(smallSize)}>{t('small text')}</Button>
          <Button onClick={() => updateTheme(mediumSize)}>{t('medium text')}</Button>
        </div>
